...
public:
    LayerProcessor(std::function<void(uint64_t*, uint64_t**)> layerFunc)
        : layerFunc(layerFunc), stop(false), active(false) {}
    ...
    void start() {
        worker = std::thread([this]() {
            while (true) {
                ...
                layerFunc(input, &output);
                if (nextLayerProcessor) {
                    nextLayerProcessor->addTask(output);
                } else {
                    delete[] output; // Cleanup if no next layer
                }
                active = false;
            }
        });
...
class MainProcessor {
public:
    MainProcessor(int id, const std::vector<std::function<void(uint64_t*, uint64_t**)>>& layerFuncs, 
                  std::vector<std::condition_variable*>& globalCVs, std::mutex& globalMtx, std::atomic<int>& activeProcessors)
        : id(id), currentLayer(0), layerFuncs(layerFuncs), globalCVs(globalCVs), globalMtx(globalMtx), activeProcessors(activeProcessors) {
        for (size_t i = 0; i < layerFuncs.size(); ++i) {
            auto layerProcessor = std::make_shared<LayerProcessor>(layerFuncs[i]);
            if (i > 0) {
                layerProcessors[i - 1]->setNextLayerProcessor(layerProcessor.get());
            }
            layerProcessors.push_back(layerProcessor);
        }
    }
    ...
    void startProcessing() {
        std::thread([this]() {
            for (currentLayer = 0; currentLayer < layerProcessors.size(); ++currentLayer) {
                {
                    std::unique_lock<std::mutex> lock(globalMtx);
                    globalCVs[id]->wait(lock, [this]() { return id == 0 || !layerProcessors[currentLayer]->isActive(); });
                }

                for (auto& task : tasks) {
                    layerProcessors[currentLayer]->addTask(task);
                }

                layerProcessors[currentLayer]->start();
                layerProcessors[currentLayer]->stopProcessing();

                if (currentLayer + 1 < layerProcessors.size()) {
                    {
                        std::unique_lock<std::mutex> lock(globalMtx);
                        globalCVs[id + 1]->notify_one();
                    }
                }
            }

            // Notify the next processor to start
            if (id + 1 < globalCVs.size()) {
                {
                    std::unique_lock<std::mutex> lock(globalMtx);
                    globalCVs[id + 1]->notify_one();
                }
            ...
            // Decrement the active processor counter
            activeProcessors -= 1;
        }).detach();
    }

    void stopAll() {
        for (auto& lp : layerProcessors) {
            lp->stopProcessing();
        }
    }
...
private:
    int id;
    size_t currentLayer;
    std::vector<std::shared_ptr<LayerProcessor>> layerProcessors;
    std::vector<uint64_t*> tasks;
    std::vector<std::function<void(uint64_t*, uint64_t**)>> layerFuncs;
    std::vector<std::condition_variable*>& globalCVs;
    std::mutex& globalMtx;
    std::atomic<int>& activeProcessors;
};
...
int main(int argc, char **argv) {
  ...
  std::vector<std::function<void(uint64_t*, uint64_t**)>> layerFuncs = {
    ...
    layer18, layer19, layer20, layer21, layer22
  };

    std::vector<std::condition_variable*> globalCVs(batch_size);
    for (int i = 0; i < batch_size; ++i) {
        globalCVs[i] = new std::condition_variable();
    }
    std::mutex globalMtx;
    std::atomic<int> activeProcessors(batch_size);

    std::vector<std::unique_ptr<MainProcessor>> mainProcessors;
    for (int i = 0; i < batch_size; ++i) {
        mainProcessors.push_back(std::make_unique<MainProcessor>(i, layerFuncs, globalCVs, globalMtx, activeProcessors));
    }

    // Load images and add them to the main processors
    std::vector<uint64_t*> images = loadInput(batch_size);
    for (int i = 0; i < batch_size; ++i) {
        mainProcessors[i]->addTask(images[i]);
    }

    // Start processing
    for (int i = 0; i < batch_size; ++i) {
        mainProcessors[i]->startProcessing();
    }

    // Wait for all main processors to finish
    while (activeProcessors > 0) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    // Clean up condition variables
    for (auto cv : globalCVs) {
        delete cv;
    }
